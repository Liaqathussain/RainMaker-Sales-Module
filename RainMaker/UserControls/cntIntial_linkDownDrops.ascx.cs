...
namespace RainMaker.UserControls
{
    public partial class cntIntial_linkDownDrops : System.Web.UI.UserControl
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (Session["Complain_Flag"].ToString() == "View")
                {
                    if (!IsPostBack)
                    {
        ...
        public bool CheckAnyValueIsEmpty(Panel pnl)
        {
            try
            {
                RadioButtonList rb = pnl.Controls.OfType<RadioButtonList>().FirstOrDefault();
                string rd = rb.SelectedValue;
                if (rd == null)
                {
                    return false;
                }
        ...
        public string GetTroubleShootValue(Panel pnl)
        {
            try
            {
                ...
                RadioButtonList rb = pnl.Controls.OfType<RadioButtonList>().FirstOrDefault();
                string rd = rb.SelectedValue.ToString();
                if (rd == null)
                {
                    val = string.Empty;
                }
